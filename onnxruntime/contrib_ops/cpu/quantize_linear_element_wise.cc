...
REG_QUANTIZE_LINEAR_ELEMENTWISE_TYPED_KERNEL(QLinearAdd, 1, int8, QLinearAdd)
REG_QUANTIZE_LINEAR_ELEMENTWISE_TYPED_KERNEL(QLinearAdd, 1, uint8, QLinearAdd)

REG_QUANTIZE_LINEAR_ELEMENTWISE_TYPED_KERNEL(QLinearMul, 1, int8, QLinearMul)
REG_QUANTIZE_LINEAR_ELEMENTWISE_TYPED_KERNEL(QLinearMul, 1, uint8, QLinearMul)
...
void QLinearBroadcastLoopSpan(TBroadcaster& bc, Output& output, Input0Scalar input0scalar, Input1Scalar input1scalar, General general,
                              float a_scale, float b_scale, float c_scale, T a_zero, T b_zero, T c_zero) {
  if (bc.IsInput0Scalar()) {
    while (output)
      input0scalar(output.NextSpanOutput(), bc.NextScalar0(), bc.NextSpan1(), a_scale, b_scale, c_scale, a_zero, b_zero, c_zero);
  } else if (bc.IsInput1Scalar()) {
    while (output)
      input1scalar(output.NextSpanOutput(), bc.NextSpan0(), bc.NextScalar1()), a_scale, b_scale, c_scale, a_zero, b_zero, c_zero;
  } else {
    while (output)
      general(output.NextSpanOutput(), bc.NextSpan0(), bc.NextSpan1(), a_scale, b_scale, c_scale, a_zero, b_zero, c_zero);
  }
}
  ...
  return QLinearBroadcastTwo<T>(
      *context,
      [](EigenVectorMap<T> output, T input0, ConstEigenVectorMap<T> input1, float a_scale, float b_scale, float c_scale, T a_zero, T b_zero, T c_zero) {
        ...
        output = (((input1.array() - b_zero) * b_scale) + a_value) / c_scale - c_zero;
      },
      [](EigenVectorMap<T> output, ConstEigenVectorMap<T> input0, T input1, float a_scale, float b_scale, float c_scale, T a_zero, T b_zero, T c_zero) {
        ...
        output = (((input0.array() - a_zero) * a_scale) + b_value) / c_scale - c_zero;
      },
      [](EigenVectorMap<T> output, ConstEigenVectorMap<T> input0, ConstEigenVectorMap<T> input1, float a_scale, float b_scale, float c_scale, T a_zero, T b_zero, T c_zero) {
        output = (((input0.array() - a_zero) * a_scale) + ((input1.array() - b_zero) * b_scale)) / c_scale - c_zero;
      });
}

template <typename T>
Status QLinearMul<T>::Compute(OpKernelContext* context) const {
  return QLinearBroadcastTwo<T, T>(
      *context,
      [](EigenVectorMap<T> output, T input0, ConstEigenVectorMap<T> input1, float a_scale, float b_scale, float c_scale, T a_zero, T b_zero, T c_zero) {
        float a_value = a_scale * (input0 - a_zero);
        output = (((input1.array() - b_zero) * (b_scale * a_value)) / c_scale - c_zero;
      ...
      [](EigenVectorMap<T> output, ConstEigenVectorMap<T> input0, ConstEigenVectorMap<T> input1, float a_scale, float b_scale, float c_scale, T a_zero, T b_zero, T c_zero) {
        output = ((input0.array() - a_zero) * a_scale).cwiseProduct((input1.array() - b_zero) * b_scale) / c_scale - c_zero;
      });
}
